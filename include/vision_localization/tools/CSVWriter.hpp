...
class CSVWriter {
  public:
    ...
    CSVWriter &add(const char *str) { return this->add(string(str)); }

    CSVWriter &add(char *str) { return this->add(string(str)); }
    ...
    void operator+=(CSVWriter &csv) { this->ss << endl << csv; }

    string toString() { return ss.str(); }
    ...
    bool writeToFile(string filename) { return writeToFile(filename, false); }
